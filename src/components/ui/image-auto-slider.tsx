...
export const Component: React.FC<ImageAutoSliderProps> = ({ images: customImages, className = "" }) => {
  ...
  const defaultImages = [
    ...
    "https://images.unsplash.com/photo-1524799526615-766a9833dec0?q=80&w=1974&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D"
  ];

  const imagesList = customImages && customImages.length > 0 ? customImages : defaultImages;
  ...
  return (
    <>
      ...
      <div className={`w-full bg-transparent relative overflow-hidden flex items-center justify-center ${className}`}>
            ...
            <div className="infinite-scroll flex gap-6 w-max">
              {duplicatedImages.map((image, index) => (
                ...
                >
                  <img
                    src={image}
                    alt={`Gallery image ${(index % imagesList.length) + 1}`}
...
export const ImageAutoSlider = Component;
